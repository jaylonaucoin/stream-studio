...
describe('SegmentMetadataForm', () => {
  ...
  it('renders shared artist helper text when useSharedArtistForSegments is true', () => {
    renderWithMui(
      <SegmentMetadataForm {...defaultProps} useSharedArtistForSegments={true} />
    )

    expect(
      screen.getByText('This artist will be applied to all segments')
    ).toBeInTheDocument()
  })
  ...
  it('calls onSegmentMetadataChange when editing album field', async () => {
    ...
    renderWithMui(
      <SegmentMetadataForm
        {...defaultProps}
        onSegmentMetadataChange={onSegmentMetadataChange}
      />
    )
