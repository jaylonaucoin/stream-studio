...
describe('useKeyboardShortcuts', () => {
  ...
  it('invokes handler for ctrl+k style combo', () => {
    const handler = vi.fn();
    render(<Harness shortcuts={{ 'ctrl+k': handler }} enabled />);
  ...
  it('does nothing when disabled', () => {
    const handler = vi.fn();
    render(<Harness shortcuts={{ k: handler }} enabled={false} />);
  ...
  it('invokes handler for meta+key combo', () => {
    const handler = vi.fn();
    render(<Harness shortcuts={{ 'ctrl+j': handler }} enabled />);
  ...
  it('cleanup removes listener on unmount', () => {
    const handler = vi.fn();
    const { unmount } = render(<Harness shortcuts={{ k: handler }} enabled />);
    unmount();
    act(() => {
      window.dispatchEvent(new KeyboardEvent('keydown', { key: 'k', bubbles: true }));
    });
