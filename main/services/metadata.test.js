...
function makeFakeHttpRequest(response) {
  ...
  req.setTimeout = vi.fn((_, cb) => { req._timeoutCb = cb })
  req.destroy = vi.fn()
  setTimeout(() => {
...
describe('fetchThumbnailBuffer', () => {
  ...
  it('rejects on network error', async () => {
    ...
    await expect(fetchThumbnailBuffer('https://example.com/fail.jpg'))
  ...
  it('rejects on timeout', async () => {
    ...
    await expect(fetchThumbnailBuffer('https://example.com/slow.jpg'))
      .rejects.toThrow('Timeout fetching thumbnail')
  })
})
