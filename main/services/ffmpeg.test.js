...
describe('convertLocalFile', () => {
  it('builds correct args for audio mp3 mode', async () => {
    const proc = makeFakeProc()
    spawnMock.mockReturnValueOnce(proc)
    const promise = convertLocalFile('/in.wav', '/out.mp3', { mode: 'audio', format: 'mp3', quality: 'best' })
    closeProc(proc, 0)
    await expect(promise).resolves.toBeUndefined()
    const args = spawnMock.mock.calls[0][1]
  ...
  it('invokes progress callback from stderr output', async () => {
    ...
    const promise = convertLocalFile('/in.wav', '/out.mp3', { mode: 'audio', format: 'mp3' }, onProgress)
