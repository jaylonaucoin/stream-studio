...
describe('convert', () => {
  ...
  it('sends playlist progress for playlist mode', async () => {
    ...
    const hasPlaylistProgress = progressCalls.some(([, data]) => data.type === 'playlist-progress');
...
describe('convertLocalFile', () => {
  ...
  it('resolves on successful delegation to ffmpeg', async () => {
    ...
    expect(addToHistory).toHaveBeenCalled();
    expect(showNotification).toHaveBeenCalledWith('Conversion Complete', expect.any(String));
  });
});
